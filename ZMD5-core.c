...
void ZMD5_Feed(const unsigned char* iData, size_t iLength) {
    static size_t lIndexData;
    static size_t lCountFill;
    static size_t lIndexBuffer;
    lIndexBuffer = (rZMD5_Counter[0] >> 3) % 64;
    lCountFill = 64 - lIndexBuffer;
    if (iLength >= lCountFill) {
        for (size_t lCopy = 0; lCopy < lCountFill; lCopy++) {rZMD5_Buffer[lIndexBuffer + lCopy] = iData[lCopy];}
        lIndexBuffer = 0;
        ZMD5_Transform((const unsigned int*)rZMD5_Buffer);
        lIndexData = lCountFill;
        while ((lIndexData + 64) <= iLength) {ZMD5_Transform((const unsigned int*)&iData[lIndexData]); lIndexData += 64;}
    } else {
		lIndexData = 0;
	}
    for (size_t lCopy = 0; lCopy < (iLength - lIndexData); lCopy++) {rZMD5_Buffer[lIndexBuffer + lCopy] = iData[lIndexData + lCopy];}
